...
int main() {
    ...
    int offset = 0;
    while (true) {
        auto n = ::read(fd, reinterpret_cast<void *>(&buf), 1);
        if (n > 0) {
            switch (offset) {
                case 0:
                    if (buf == 0x54) {
                ...
                case 3:
                    rx_buf[offset] = buf;
                    if (crc8(reinterpret_cast<uint8_t*>(&rx_buf[0]), 3) == rx_buf[3]) {
                        auto distance = ((0xff & rx_buf[1]) << 8) + (0xff & rx_buf[2]);
                        std::cout << distance * 0.001 << "m" << std::endl;
                    } else {
